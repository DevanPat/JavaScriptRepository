...
// am checking for wincon squares 678
function checkWinCon3(info,squareArray) {
	var winCon3 = [6,7,8];
	var winDetected = "on";
	for (var i in info) {
		if (info[i].charAt(0) == "6") {
			var match6Avatar = info[i].charAt(1); // only want to record the avatar
		}
...
// am checking for wincon squares 036
function checkWinCon4(info,squareArray) {
	var winCon4 = [0,3,6];
	var winDetected = "on";
	for (var i in info) {
		if (info[i].charAt(0) == "0") {
			var match0Avatar = info[i].charAt(1); // only want to record the avatar
		}
		if (info[i].charAt(0) == "3") {
...
// am checking for wincon squares 147
function checkWinCon5(info,squareArray) {
	var winCon5 = [1,4,7];
	var winDetected = "on";
	for (var i in info) {
		if (info[i].charAt(0) == "1") {
			var match1Avatar = info[i].charAt(1); // only want to record the avatar
		}
		if (info[i].charAt(0) == "4") {
			var match4Avatar = info[i].charAt(1);
		}
...
// am checking for the wincon squares 258
...
// im checking for wincon squares 642
...
// am checking wincon squares 048
...
//-----------------------------------------------------------------------------------------
//  These functions are for the click events and the square element that corresponds with them
//--------------------------------------------------------------------------------------------
function square1Animate() {
	var activePlayer = document.getElementById('showPlayer').innerHTML;
	if (activePlayer != "Game Stopped") { // if the game hasn't started yet stop the avatar placement
		var square = "0"; // identify the selected square
		// check and see if the proposed square is valid
		var verdict = recordMoves(square);
		if (verdict == undefined) { // if verdict is empty than square is unoccupied.
			var paintAvatar = determineAvatar(); // get correct avatar to paint for active player
			var selected = document.getElementsByClassName(paintAvatar)[0]; // paint the avatar
			if (paintAvatar == "O") { // change all to ternary satetments 
				animateO(selected); // call function animate O
			} else if (paintAvatar == "X") {
				animateX(selected); // call function animate X
			}
			// build new array that adds newly selected square and assigned avatar
			var currentMove = ","+square+paintAvatar;
			recordMove(currentMove);
			checkForWinCon(); // call function and check if current move completes winning condition.
			avatarPlaced(square,paintAvatar); // end the current turn and pass the turn to other player
